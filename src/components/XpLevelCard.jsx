...
export function XpLevelCard({ selected = false }) {
  ...
  return (
    <section className={`relative overflow-hidden rounded-xl border p-5 shadow-[var(--card-shadow)] ${selected ? '!border-[#c4ff0e] !bg-[#c4ff0e] !text-black' : 'border-neutral-800 bg-neutral-900/80 text-[var(--app-text)]'}`}>
      <div className="flex items-end justify-between gap-4">
        <div>
          <div className={`text-[10px] uppercase tracking-[0.7em] ${selected ? '!text-black/60' : 'text-neutral-500'}`}>[ THE PULSE ]</div>
          <h2 className={`mt-2 font-serif text-3xl uppercase tracking-[0.08em] ${selected ? '!text-black' : 'text-[var(--app-text)]'}`}>XP / Level</h2>
        </div>
