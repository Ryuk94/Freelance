...
export function HuntCard({ selected = false }) {
  ...
  return (
    <section className={`h-full rounded-xl border p-5 ${selected ? '!border-[#c4ff0e] !bg-black !text-[#c4ff0e]' : 'border-neutral-800 bg-white/[0.03]'}`}>
      <div className="mb-4 relative">
        <div className={`text-[10px] uppercase tracking-[0.7em] ${selected ? 'text-[#c4ff0e]/60' : 'text-neutral-500'}`}>[ THE HUNT ]</div>
      ...
      <div className="space-y-2">
        {huntQueue.length === 0 ? (
            <div className={`px-4 py-3 text-xs uppercase tracking-[0.45em] ${selected ? 'border border-[#c4ff0e]/20 bg-black text-[#c4ff0e]/70' : 'bg-black/40 text-neutral-500'}`}>
              queue empty
            </div>
        ) : (
          huntQueue.map((lead) => {
            ...
            return (
              <label
                ...
                className={[
                  ...
                  : selected
                      ? 'bg-black text-[#c4ff0e]/85 hover:bg-black/80 hover:text-[#c4ff0e]'
              ...
              >
                <input
                  ...
                  className={`h-5 w-5 rounded-none border ${selected ? 'border-[#c4ff0e]/30 bg-black text-[#c4ff0e] accent-[#c4ff0e]' : 'border-[#c4ff0e]/40 bg-black text-[#c4ff0e] accent-[#c4ff0e]'}`}
                />
                <div className="min-w-0">
                  <div className="truncate text-sm font-bold uppercase tracking-[0.35em] text-inherit">
                    {lead.companyName}
                  </div>
                  <div className={`mt-1 text-[10px] uppercase tracking-[0.45em] ${selected ? 'text-[#c4ff0e]/55' : 'text-neutral-500'}`}>
